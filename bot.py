...
def cmd_receipt(message):
    """Команда для приема чека (только для админа или агента)"""
    ...
    user_states[message.chat.id] = 'waiting_receipt_username'
    bot.send_message(
        message.chat.id,
        "📋 <b>ПРИЕМ ЧЕКА НА ЗАЧИСЛЕНИЕ</b>\n\n"
        "Введите username клиента (без @):",
        parse_mode='HTML'
    )


@bot.message_handler(func=lambda message: user_states.get(message.chat.id) == 'waiting_receipt_username')
def process_receipt_username(message):
    """Обработка username клиента из чека"""
    chat_id = message.chat.id
    ...
    if not client_id:
        bot.send_message(chat_id, f"❌ Пользователь с username @{username} не найден!")
        return
    
    # Сохраняем найденного пользователя
    user_states[chat_id] = {
        'receipt_step': 'waiting_amount',
        'client_id': client_id,
        'username': username
    }
...
@bot.message_handler(func=lambda message: isinstance(user_states.get(message.chat.id), dict) and user_states.get(message.chat.id, {}).get('receipt_step') == 'waiting_amount')
def process_receipt_amount(message):
    ...
    try:
        ...
        bot.send_message(chat_id, "❌ Пожалуйста, введите число (например: 100 или 100.50)")
        return
    ...
    admin_id_setting = int(get_setting('ADMIN_ID', ADMIN_ID))
    bot.send_message(admin_id_setting, notification_text, parse_mode='HTML')
